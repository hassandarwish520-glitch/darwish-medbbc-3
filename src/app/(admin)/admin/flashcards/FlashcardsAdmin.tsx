...
export default function FlashcardsAdmin({ initial, lessons }: { initial: F[]; lessons: any[] }) {
...
function AddModal({ onClose, onCreated, lessons }: any) {
  ...
  async function submit() {
    setBusy(true);
    const r = await fetch("/api/admin/flashcards", { method:"POST", headers:{"Content-Type":"application/json"},
      body: JSON.stringify({ lesson_id: f.lesson_id || null, front: f.front, back: f.back,
        tags: f.tags.split(",").map(t=>t.trim()).filter(Boolean) }) });
    setBusy(false);
    if (r.ok) { const { card } = await r.json(); onCreated(card); }
  }
  return (
