...
type AddModalProps = { onClose: () => void; onCreated: (question: Q) => void; lessons: LessonOption[] };
...
function AddModal({ onClose, onCreated, lessons }: AddModalProps) {
  const [f, setF] = useState<QuestionFormState>({
    lesson_id: "",
    stem: "",
    A: "",
    B: "",
  ...
  const set = (k: keyof QuestionFormState, v: string) => setF({ ...f, [k]: v });

  async function submit() {
    setBusy(true);
    const choices = (["A", "B", "C", "D", "E"] as const)
      .filter((k) => f[k])
      .map((k) => ({ key: k, text: f[k] }));
