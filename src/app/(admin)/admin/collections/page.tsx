...
import { BookOpen, ChevronRight, FileText, Package, Search, Video, VideoOff } from "lucide-react";
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8">
      ...
      <p className="mt-2 text-base leading-7" style={{ color: "var(--c-text-3)" }}>
        اختر Collection لإضافة أو تعديل الفيديو التوضيحي لكل سؤال.
      </p>
      ...
      <div className="mt-6">
          ...
          <div className="space-y-3">
            {filtered.map((c) => (
              <Link
                ...
                className="group flex items-center gap-4 rounded-[20px] border p-5 transition-all hover:shadow-lg"
                style={{
              ...
              >
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <div
                    ...
                    style={{ background: "var(--c-brand)", color: "#fff", opacity: 0.85 }}
                  >
                    <Video className="h-3.5 w-3.5" />
                    إدارة الفيديو
                  </div>
                  <ChevronRight className="h-4 w-4" style={{ color: "var(--c-text-4)" }} />
                </div>
              </Link>
            ))}
          </div>
        ...
        {!loading && !error && filtered.length > 0 && (
          <p className="mt-6 text-center text-xs" style={{ color: "var(--c-text-4)" }}>
            ...
            {filtered.reduce((s, c) => s + c.question_count, 0).toLocaleString()} سؤال
          </p>
        )}
      </div>
    </div>
