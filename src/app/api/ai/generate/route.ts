...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (mode === "flashcards") {
      ...
      const rows = items.map((f: { front: string; back: string; tags?: string[] }) => ({
        lesson_id,
        front: f.front,
        back: f.back,
        tags: f.tags ?? [],
      ...
      await Promise.all(
        (inserted ?? []).map(async (row) => {
          const indexText = extractFlashcardIndexText(row);
          if (indexText) await indexSource("flashcard", row.id, indexText);
        })
      );
